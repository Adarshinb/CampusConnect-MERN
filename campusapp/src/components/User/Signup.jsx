...
const Signup = () => {
  const [inputs, setInputs] = useState({
    name: "", email: "", password: "", confirmPassword: "", phone: "", branch: "", year: "",
    // userType: "student" // don't allow clients to set admin in production
  });
  ...
  const signupHandler = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        year: inputs.year,
        // DO NOT send userType as admin in production; backend should enforce role assignment
      };
      ...
      // success
      // you can show snackbar instead of alert
      ...
      if (serverMsg) {
        // If server returns validation string, show under a general field
        setErrors(prev => ({ ...prev, server: serverMsg }));
      } else {
        setErrors(prev => ({ ...prev, server: "Signup failed. Try again." }));
      }
    } finally {
      setLoading(false);
    }
